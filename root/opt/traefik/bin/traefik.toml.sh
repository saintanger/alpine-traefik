...
TRAEFIK_ACME_WILDCARD_DOMAIN=${TRAEFIK_ACME_WILDCARD_DOMAIN:-"false"}
...
TRAEFIK_INFLUX_DB=${TRAEFIK_INFLUX_DB:-""}
...
"
    ...
    if [ "${TRAEFIK_ACME_WILDCARD_DOMAIN}" != "" ]; then

        TRAEFIK_ACME_CFG=${TRAEFIK_ACME_CFG}"\
  [[acme.domains]]
    main = \"*.${TRAEFIK_ACME_WILDCARD_DOMAIN}\"
    sans = [\"${TRAEFIK_ACME_WILDCARD_DOMAIN}\"]
"
    fi
